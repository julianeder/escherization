import { _TRACESKELETON } from './trace_skeleton';
...
class TraceSkeleton {
  ...
  fromImageData(im, resolution) {
    ...
    return this.fromCharString(str, w, h, resolution);
  }
  fromCanvas(im) {
  ...
  fromCharString(im, w, h, resolution) {
    var T = new this.tracer.skeleton_tracer_t();
    var s = T.trace(im, w, h, resolution);
    var r = s
      .split('RECTS:')[1]
      .split('\n')
      .filter((x) => x.length)
